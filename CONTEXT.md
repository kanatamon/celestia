...
**Follower Badge**:
The green-glass, glassmorphic thumbs-up jewel pinned to the **TOP-LEFT** of a viewer avatar in the Session Tab feed, marking a viewer who **follows the streamer**. Driven by `followStatus` on the event's `UserInfo`: a viewer who follows (`followStatus >= 1`) gets the badge; a stranger (`0`) or unknown (`undefined`) gets **no DOM node at all** — the silence is the point, so followers pop pre-attentively in a fast scroll. **Binary, no friend tier**: `followStatus` 1 (following) and 2 (mutual) are treated identically; the amber friend tint/ring/star was prototyped and explicitly cut. Coexists with the **Heart Me badge**, which is reserved for the avatar's top-right. Unlike the **Like Layer**, it reads **standing** ("does this viewer follow"), not **presence** ("who is active right now") — the two are complementary loyalty axes, not redundant. Consistent with the identity contract (ADR-0008, ADR-0010): it annotates an already-disclosed card identity and never touches the anonymous **Gift Celebration** overlay.
_Avoid_: Follower signal, green thumb, follow icon, thumbs-up, follower mark
