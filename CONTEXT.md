...
**Follower Badge**:
The flat **translucent blue disc** (hue 212) with a white thumbs-up glyph, pinned to the **TOP-LEFT** of a **ChatEventCard** avatar in the Session Tab feed, marking a viewer who **follows the streamer**. **Chat cards only** — a **GiftEventCard** avatar never carries it; the compact gift avatar opts out of the standing-badge slot entirely (ADR-0010 Amendment 2, 2026-06-07). Driven by `followStatus` on the event's `UserInfo`: a viewer who follows (`followStatus >= 1`) gets the badge; a stranger (`0`) or unknown (`undefined`) gets **no DOM node at all** — the silence is the point, so followers pop pre-attentively in a fast scroll. The pop comes from **color-block contrast** (hue + white glyph against the avatar), **not** glow: glow, border, inset highlight and backdrop-blur were all removed (ADR-0010 amendment) after the original green-glass jewel read as too distracting while reading the feed. **Binary, no friend tier**: `followStatus` 1 (following) and 2 (mutual) are treated identically; the amber friend tint/ring/star was prototyped and explicitly cut. A live follow transition (#91) **pops** the badge in (scale snap, no bloom). **Suppressed when the viewer has a Heart Me badge** — the two share one top-left slot and Heart Me wins it (ADR-0011). Unlike the **Like Layer**, it reads **standing** ("does this viewer follow"), not **presence** ("who is active right now") — the two are complementary loyalty axes, not redundant. Consistent with the identity contract (ADR-0008, ADR-0010): it annotates an already-disclosed card identity and never touches the anonymous **Gift Celebration** overlay.
_Avoid_: Follower signal, green thumb, follow icon, thumbs-up, follower mark, blue disc

**Heart Me badge**:
The "Heart Me" **Gift Icon** pinned to a viewer avatar's **TOP-LEFT**, marking a viewer who has sent the **Heart Me** gift this **Live Session** — a gift TikTok lets a follower send **only once a day**. Shares **one slot** with the **Follower Badge** and **wins it** (ADR-0011): when present, the Follower Badge is suppressed, because a Heart Me sender is by definition already a follower, so the more specific signal stands in for the generic one. Treated as a **superset signal** — its presence is trusted as evidence of standing **without re-checking `followStatus`**. Unlike the Follower Badge it is a **static image and never animates** (no "just followed" pop). Sized and offset to match the Follower Badge slot; carries no glow or shadow, consistent with the calm-at-rest direction. **Surfaced on the ChatEventCard avatar only** — a **GiftEventCard** avatar shows **no standing badge at all** (neither Heart Me nor Follower); the compact gift avatar opts out of the slot because any badge crowded it beside the gift image (ADR-0010 Amendment 2 + ADR-0011 amendment, 2026-06-07).
_Avoid_: Gift badge, badgeGift, Heart Me icon, heart badge, top-right badge
...
**Connection Advisory**:
The popover that auto-opens over the **ConnectionSignal** when the connection enters a fault the user should be aware of (the `offline` and `reconnecting` signal kinds). Explains *why* it happened and offers a workaround, branching on the **ConnectionState** `reason` (`offline | off-live | interrupted | stale`). The action depends on the reason: `offline` is informational only and auto-recovers; `off-live` offers **Reopen live** (navigates the *existing paired tab* back to this session's streamer `/@user/live` — distinct from **Reconnect**, which only re-attaches the debugger, and **Relaunch**, which opens a *new* paired tab); `interrupted` and `stale` offer **Reconnect**. For `interrupted`/`stale` faults where the paired tab is still on the live, the advisory is **suppressed while the Session Tab silently auto-reconnects** (see **Auto-Reconnect**) and only opens once those retries are exhausted. Anchored on the bars, it auto-opens once per fault episode, is dismissible, can be reopened by clicking the bars, and self-closes on recovery.
_Avoid_: Connection alert, error toast, reconnect dialog, notification

**Auto-Reconnect**:
The Session Tab's silent, best-effort recovery from a `reconnecting` fault (**ConnectionState** `reason` `interrupted` or `stale`) **while the paired tab is still on the live**. Instead of immediately opening the **Connection Advisory**, the Session Tab fires the same mechanism as a manual **Reconnect** (remount → re-attach the debugger → re-discover the socket) up to **3 times** — ~6s per attempt, ~1s between, ~20s worst case — suppressing the advisory throughout (the **ConnectionSignal** still shows "Reconnecting"). An attempt **succeeds** on reaching `connected` and **fails** by re-faulting or timing out. Reaching `connected` resets the budget to 3. If the fault changes mid-retry to `off-live` or `offline`, Auto-Reconnect **abandons** and hands off to that reason's path. Once the budget is exhausted the advisory opens with a **patient, un-timed** manual **Reconnect**; clicking it is a one-shot attempt and does **not** restart the auto cycle. The retry budget lives in the **Session Tab** (above the remount boundary), not in the Provider.
_Avoid_: Retry, auto-retry, auto-heal, reconnect loop
...
**ConnectionState**:
The externally observable state of a Provider at any point in time. `status` is one of `idle | attaching | attached | connecting | connected | detaching | disconnecting | detached | disconnected | error`. An `error` status carries a `reason`: `offline` (device lost network), `off-live` (the paired tab is still open with the debugger attached, but has navigated away from the live stream — its URL is no longer a `/@user/live` page), `interrupted` (Chrome Debugger detached — usually the "Celestia is debugging this browser" banner was dismissed), or `stale` (debugger attached but no LiveEvents past the stale threshold). `off-live` is only raised **after a confirmed-live connection** — a cold attach to a non-live tab does not raise it (the **Launcher** only opens Session Tabs for live tabs). The **ConnectionSignal** projects this onto five UI kinds; the **Connection Advisory** branches on `reason`.
