...
The Synthesized celebration does **not** reuse the triptych. It shows the Gift Icon **centred with no side gutters**, sized as a **square at 40% of the feed-contained portrait height** (`SYNTHESIZED_ICON_SCALE`, validated by `prototype-celebration-feed-viewport-scale.html`) — deliberately **smaller** than the full-bleed Animated kind, which still fills the whole contained portrait. The render path also differs from the Animated celebration:
...
- **2026-06-03 — Icon sized smaller (square, 40% of contained height).** Validated by `prototype-celebration-feed-viewport-scale.html`. The centred Gift Icon is now a **square sized to 40% of the feed-contained portrait height** (`computeSynthesizedGiftIconLayout`, `SYNTHESIZED_ICON_SCALE = 0.4`) rather than filling the full contained portrait frame, so it reads as the lighter, smaller kind beside the full-bleed Animated celebration. The load-bearing burst still spans the full stage and carries the width. Glow and fireworks origin follow the smaller square; the Animated kind is unchanged.
