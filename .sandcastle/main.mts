...
// Due to Codex subscription limitations, we limit iterations to 2.
const MAX_ITERATIONS = 2;

// Hooks run inside the sandbox before the agent starts each iteration.
// Keep startup fast; package installs should be run by agents only when needed.
const hooks = {
	sandbox: { onSandboxReady: [{ command: 'true' }] },
};

// Copy node_modules from the host into the worktree before each sandbox
// starts. Avoids a full package install from scratch in implementation sandboxes.
const copyToWorktree = ['node_modules'];

// Codex CLI reads subscription/auth state from ~/.codex/auth.json. Mount the
// host auth file read-only instead of passing OPENAI_KEY through the env.
