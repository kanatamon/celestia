...
export const LiveStatusBadge: React.FC<{ username: string }> = ({
	...
	const closeErrorInspection = () => {
		setIsErrorInspectionOpen(false);
	};

	const openErrorInspection = () => {
		setIsErrorInspectionOpen(true);
	};
	...
	return (
		<>
									...
									onClick={() => {
										GlassModal.confirm({
											...
											content: (
												<Flex vertical gap={6}>
													<Paragraph
														style={{ color: 'currentcolor', margin: 0 }}
													>
														You'll lose all chat history, likes, and gift data
														for this session. And this action can't be undone.
													</Paragraph>
												</Flex>
											),
											okText: 'Clear Chat',
										...
										GlassModal.confirm({
											...
											content: (
												<Flex vertical gap={6}>
													...
													>
														You'll lose all chat history, likes, and gift data
														for this session. The live stream will stop and this
			...
			<LiveConnectionAlert
				username={username}
				isOpen={isErrorInspectionOpen}
				onClose={closeErrorInspection}
			/>
		</>
	);
};
